...
(define (myeval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((define-ref? exp) (lookup-dynamic exp env))
...
(define (extend-dynamic-environment vars vals dynamic-environment)
  (if (and (null? vars) (null? vals))
      dynamic-environment
      (if (= (length vars) (length vals))
          (if (eq? (car vars) 'dynamic)
              (cons (cons (cadr vars) (car vals)) (extend-dynamic-environment (cdr vars) (cdr vals) dynamic-environment))
              (extend-dynamic-environment (cdr vars) (cdr vals) dynamic-environment))
          (if (< (length vars) (length vals))
              (error "Too many arguments supplied" vars vals)
              (error "Too few arguments supplied" vars vals)))))

(define (lookup-dynamic exp env)
  (if (not (eq? ("Unbound variable" (lookup-variable-value (cadr exp) dynamic-environment))))
      (lookup-variable-value (cadr exp) dynamic-environment)
      (lookup-variable-value (cadr exp) env)))

(define (define-ref? exp) (tagged-list? exp 'define-ref))
